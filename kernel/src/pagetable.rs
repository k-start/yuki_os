...
pub unsafe fn active_level_4_table() -> &'static mut PageTable {
    ...
    &mut *((p4 + memory_info.phys_mem_offset.as_u64()) as *mut PageTable)
}
...
//     let virt = memory_info.phys_mem_offset + phys.as_u64();
...
impl PageTable {
    pub unsafe fn new() -> Box<PageTable> {
        ...
        let cur_pt0 = active_level_4_table().entries[0].next_pt();
...
impl VirtAddr {
    ...
    pub unsafe fn to_phys(&self) -> Option<(PhysAddr, &PTEntry)> {
...
impl PhysAddr {
    ...
    pub unsafe fn to_virt(&self) -> Option<VirtAddr> {
        ...
        if self.0 < 0x100000000 {
            Some(VirtAddr::new(self.0 + memory_info.phys_mem_offset.as_u64()))
        } else {
            None
        }
    }
